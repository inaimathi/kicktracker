...
(defn ks-time->datetime [long]
  (clj-time.coerce/from-long (* long 1000)))

(defn query-project [proj]
  {:id (proj "id")
   :url (gets proj "urls" "web" "project")
   :launched-at (proj "launched_at")
       ...
       (hic/html
        [:feed {:xmlns "http://www.w3.org/2005/Atom"}
         ...
         (map
          (fn [p]
            [:entry 
             [:title (p :name)] [:link {:href (p :url)}] [:id (str "project-" (p :id))]
             [:updated (ks-time->datetime (p :launched-at))]
             [:summary (p :blurb)]
             [:author [:name (p :creator)]]])
          (sort-by :launched-at > (map query-project projects)))])))

(defn handler [req]
  (let [body (case (req :uri)
               ("/" "/recently-launched") (projects->feed "Recent" "All recently launched projects" ((recently-launched) "projects"))
               "/staff-picks" (projects->feed "Picks" "Recent KS staff picks" ((staff-picks) "projects"))
               "/board-games" (projects->feed "Boardgames" "Recently launched tabletop game projects" ((specific "tabletop games") "projects"))
               nil)]
    (if body
      {:status 200
...
(defn -main 
  ([]     (-main "8000"))
  ([port] (server/run-server handler {:port (read-string port)})))
